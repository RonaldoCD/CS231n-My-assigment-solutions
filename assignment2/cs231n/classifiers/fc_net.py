...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a1, cache1 = affine_relu_forward(x_reshaped, W1, b1)
        ...
        loss_reg = self.reg * 0.5 * (np.sum(W1 * W1) + np.sum(W2 * W2))
...
#         tic = time.time()
        ...
        for i in range(self.num_layers - 1):
            ...
            z, cache_affine  = affine_forward(a, W, b)
            
            if self.normalization == "batchnorm":
                gamma = self.params['gamma'+str(i+1)]
                beta = self.params['beta'+str(i+1)]
                z , cache_batchnorm = batchnorm_forward(z, gamma, beta, self.bn_params[i])
                caches_dict['batchnorm' + str(i+1)] = cache_batchnorm
                
            a, cache_relu = relu_forward(z)
            caches_dict['layer' + str(i+1)] = (cache_affine, cache_relu)
            
            if self.use_dropout:
                a, cache_dropout = dropout_forward(a, self.dropout_param)
                caches_dict['dropout' + str(i+1)] = cache_dropout
        
        W = self.params['W'+str(self.num_layers)]
        b = self.params['b'+str(self.num_layers)]
        scores, scores_cache = affine_forward(a, W, b)
        ...
        loss_data, dscores = softmax_loss(scores, y)
        da, dW, db = affine_backward(dscores, scores_cache)
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if self.use_dropout:
                cache_dropout = caches_dict['dropout' + str(i-1)]
                da = dropout_backward(da, cache_dropout)
                
            cache_affind, cache_relu = caches_dict['layer' + str(i - 1)]
            da = relu_backward(da, cache_relu)
            
            if self.normalization == "batchnorm":
                cache_batchnorm = caches_dict['batchnorm' + str(i - 1)]
                da, dgamma, dbeta = batchnorm_backward(da, cache_batchnorm)
                grads['gamma' + str(i-1)] = dgamma
                grads['beta' + str(i-1)] = dbeta
            
            da, dW, db = affine_backward(da, cache_affind)
