...
def content_loss(content_weight, content_current, content_original):
    ...
    cont_loss = content_weight * torch.sum((content_current - content_original)** 2)
    return cont_loss
...
def gram_matrix(features, normalize=True):
    ...
    G_matrix = torch.zeros(N, C, C) 
    
    for n in range(N):
        features_n = features[n]
        G_matrix_n = torch.mm(features_n, features_n.T)
        G_matrix[n] = G_matrix_n
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for l in style_layers:
        features_l = feats[l]
        gen_gram_matrix = gram_matrix(features_l)
        style_gram_matrix = style_targets[i]
        style_loss += style_weights[i] * torch.sum((gen_gram_matrix - style_gram_matrix) ** 2)
        i+=1
