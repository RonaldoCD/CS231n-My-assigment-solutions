...
def sample_noise(batch_size, dim, seed=None):
    ...
    img_noise = torch.rand((batch_size, dim)) * 2 - 1
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    size_real = logits_real.shape
    target_real = torch.ones(size_real).type(dtype)
    
    size_fake = logits_fake.shape
    target_fake = torch.zeros(size_fake).type(dtype)
...
def generator_loss(logits_fake):
    ...
    target = torch.ones(size).type(dtype)
